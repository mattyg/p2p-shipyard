...
export function injectHolochainClientEnv(appId: String, appWebsocketPort: number, appToken: Uint8Array) {
  ...
  (window as any).__HC_ZOME_CALL_SIGNER__ = {
    signZomeCall: async (request: CallZomeRequest): Promise<CallZomeRequestSigned> => {
        ...
        const response = await (window as any).__TAURI_INTERNALS__.invoke("plugin:holochain-foreground-service|sign_zome_call", zomeCallUnsigned);
